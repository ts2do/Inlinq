...
namespace Inlinq.Impl
{
    public struct InlinqArray<T> : IEnumerable<T, ArrayEnumerator<T>>, IList<T>, IReadOnlyList<T>
    {
        ...
        public T this[int index]
        {
            get
            {
                if (index.GeUn(length)) throw Error.ArgumentOutOfRange(nameof(index));
        ...
        public void CopyTo(T[] array, int arrayIndex) => Array.Copy(this.array, startIndex, array, arrayIndex, length);

        public ArrayEnumerator<T> GetEnumerator() => new ArrayEnumerator<T>(array, startIndex, length);

        public int IndexOf(T item) => Array.IndexOf(array, item, startIndex, length);
